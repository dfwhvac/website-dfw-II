...
| 4b | **P2.16** | **React Compiler hook-rule errors surfaced by ESLint v9 migration** — 6 real errors in `AddressAutocomplete.jsx`, `RealWorkWidget.jsx`, `ServiceTemplate.jsx` (2 instances), `StickyMobileCTA.jsx`, `TestimonialCarousel.jsx`. Categories: "Cannot access refs during render", "Calling setState synchronously within an effect", "Cannot create components during render". May be real performance bugs OR false positives from `eslint-plugin-react-hooks@7.x`'s new compiler analysis. Run `yarn lint` to see exact line numbers. Per-file analysis required. | 1–2 hrs | Agent |
| 4c | **P2.18** | **Next.js 16 `cacheComponents` exploration** — Now unlocked by the May 13 Phase B upgrade. The opt-in flag (set in `next.config.js` under `experimental.cacheComponents`) lets a single component mix cached and dynamic UI without manually wrapping in `<Suspense>`. Target use: homepage hero (currently fully ISR'd at `revalidate=3600`) — selectively cache the static H1/marketing copy while letting `<LeadForm />` stay dynamic. Expected payoff: another 100–200ms off LCP, since the hero block can hydrate from edge cache while the form fetches its reCAPTCHA token in parallel. Approach: 30-min spike to enable the flag locally + measure LCP delta on preview, then decide whether to ship. If it works for hero, evaluate applying to city-page templates next. | 30 min spike + 1–2 hrs ship | Agent |
