...
- ✅ **KPI Dashboard v3 redesign (May 12)** — Card grid → 6-column compact NOC table. Row height ~80px → ~32px (single-line). Source moved to Current-cell hover tooltip; detail context moved to Metric-cell hover tooltip; V3 Pillar column removed. Added Updated column (relative time), functional filter chips (All/Red/Yellow/Green/Pending), substring search, and sticky table header. Whole 53-metric roadmap now visible in ~2 scrolls instead of ~6.
