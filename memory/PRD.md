...
- ✅ **CDN Edge Hit Rate fix (May 11)** — Flipped 19 pages from `force-dynamic` to `revalidate = 3600` ISR. Expected: 53% → 90%+ hit rate, TTFB <50ms on cached pages, ~70% reduction in Vercel serverless invocations.
  - ⚠️ **Verification incomplete — KPI dashboard shows CDN Edge Hit Rate did NOT flip green after deploy.** Vercel build succeeded after fixing the lucide-react brand-icon prerender crash (inline SVG workaround in Footer.jsx). KPI Audit workflow ran successfully post-deploy, but the CDN Edge Hit Rate card is still red. **Troubleshoot on next session resume.** Likely hypotheses to investigate: (a) Vercel edge regions hadn't fully propagated the new deploy when KPI workflow ran, (b) Next.js middleware or response headers (cookies, set-cookie) are forcing cache bypass despite the page-level `revalidate`, (c) the `x-vercel-cache` header sampling method in `scripts/audit-kpis.mjs` has a timing/warm-up bug, or (d) ISR for the specific pages sampled (`/`, `/about`, `/services`, `/reviews`, `/financing`, `/faq`, `/cities-served/dallas`) is being defeated by something route-specific.
